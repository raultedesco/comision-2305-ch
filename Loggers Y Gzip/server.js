...
app.get('*', (req, res) => {
  ...
  res.send(`Ruta ${method} ${url} inexistente`)
})
...
if(modo == "cluster"){
    const cpus = os.cpus().length //- Cantidad de procesadores

    if (cluster.isMaster) {
        console.log('Ingreso en modo cluster')
        ...
        const connectedServer = httpServer.listen(PORT, function () {
            console.log(`Servidor express en ${connectedServer.address().port} - PID ${process.pid} - ${ new Date() }`)
        })
        connectedServer.on('error', error => console.log(`Error en servidor ${error}`))
    }
}
else{ //modo fork
    console.log('Ingreso en modo fork')
    const PORT = args.port || 8080
    const connectedServer = httpServer.listen(PORT, function () {
        console.log(`Servidor express en ${connectedServer.address().port} - PID ${process.pid} - ${ new Date() }`)
    })
